...
1. 配置谷歌学术引用爬虫：
    ...
    1. 在github本仓库页面的`Action`中，点击*"I understand my workflows, go ahead and enable them"*启用workflows。此后你可以在 `Get Citation Data` workflow 页面中手动运行该 action。它会将谷歌学术引用统计数据 `gs_data.json` 更新到本仓库的 `google-scholar-stats` 分支中。
